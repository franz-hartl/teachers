...
query UnitVolumeQuery($path: String, $navPath: String) {
  ...
  nav: allMarkdownRemark(filter: {frontmatter: {path: {regex: $navPath}}}
...
const getVolNavData = (nav) => {
  ...
  var volNavItems = []
  volNavItems.push({ path: nav[itemsCount - 1].node.frontmatter.path, title: nav[itemsCount - 1].node.frontmatter.unitTitle, author: nav[itemsCount - 1].node.frontmatter.unitAuthor })
  volNavItems.push({ path: nav[itemsCount - 2].node.frontmatter.path, title: nav[itemsCount - 2].node.frontmatter.unitTitle, author: nav[itemsCount - 2].node.frontmatter.unitAuthor })

  for (let i = 0; i <= nav.length - 2; i++) {
    volNavItems.push({ path: nav[i].node.frontmatter.path, title: nav[i].node.frontmatter.unitTitle, author: nav[i].node.frontmatter.unitAuthor })
  }
  return volNavItems
}